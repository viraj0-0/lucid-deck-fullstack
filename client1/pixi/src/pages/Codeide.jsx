...
import { 
  Play, Terminal as TerminalIcon, Trash2, Cpu, Loader2, 
  Sparkles, Bug, FileText, Activity, MessageSquare, ShieldCheck, 
  GraduationCap, Code2, Zap, Download, User, Hash, 
  Sun, Moon, ZoomIn, ZoomOut, History, Clock, X, RotateCcw,
  Globe, Layout // Added icons for Web Mode
} from 'lucide-react';
...
const syntaxPatterns = {
  python: [
    ...
    { type: 'keyword', regex: /\b(def|class|if|elif|else|while|for|try|except|import|from|return|print|in|and|or|not|as|pass|break|continue|None|True|False|with|lambda|global|nonlocal|raise|yield|async|await)\b/g },
  ...
  javascript: [
    ...
    { type: 'keyword', regex: /\b(const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|import|export|default|try|catch|finally|class|extends|new|this|super|async|await|typeof|void|null|undefined|true|false)\b/g },
    { type: 'function', regex: /\b[a-zA-Z_][a-zA-Z0-9_]*(?=\()/g },
    { type: 'number', regex: /\b\d+(\.\d+)?\b/g },
  ],
  html: [
    { type: 'comment', regex: /<!--[\s\S]*?-->/g },
    { type: 'string', regex: /"[^"]*"|'[^']*'/g },
    { type: 'tag', regex: /<\/?[a-z][a-z0-9-]*\b[^>]*>/ig }, // HTML Tags
    { type: 'keyword', regex: /\b(DOCTYPE|html|head|body|div|span|h1|h2|h3|p|a|ul|ol|li|script|style|link|meta)\b/ig },
  ],
  cpp: [
    { type: 'comment', regex: /(\/\/.*|\/\*[\s\S]*?\*\/)/g },
    { type: 'string', regex: /"[^"]*"/g },
    { type: 'keyword', regex: /\b(int|float|double|char|void|bool|if|else|for|while|do|switch|case|break|continue|return|class|struct|public|private|protected|static|const|virtual|override|namespace|using|include|true|false|nullptr|new|delete|cout|cin|endl|std)\b/g },
    { type: 'function', regex: /\b[a-zA-Z_][a-zA-Z0-9_]*(?=\()/g },
    { type: 'number', regex: /\b\d+(\.\d+)?\b/g },
  ],
  c: [
    { type: 'comment', regex: /(\/\/.*|\/\*[\s\S]*?\*\/)/g },
    { type: 'string', regex: /"[^"]*"/g },
    { type: 'keyword', regex: /\b(int|float|double|char|void|if|else|for|while|do|switch|case|break|continue|return|struct|static|const|sizeof|include|printf|scanf)\b/g },
    { type: 'function', regex: /\b[a-zA-Z_][a-zA-Z0-9_]*(?=\()/g },
    { type: 'number', regex: /\b\d+(\.\d+)?\b/g },
  ],
  java: [
    { type: 'comment', regex: /(\/\/.*|\/\*[\s\S]*?\*\/)/g },
    { type: 'string', regex: /"[^"]*"/g },
    { type: 'keyword', regex: /\b(public|private|protected|class|interface|enum|extends|implements|static|final|void|int|double|boolean|char|if|else|for|while|do|switch|case|break|continue|return|try|catch|finally|throw|throws|new|this|super|import|package|true|false|null)\b/g },
    { type: 'function', regex: /\b[a-zA-Z_][a-zA-Z0-9_]*(?=\()/g },
    { type: 'number', regex: /\b\d+(\.\d+)?\b/g },
  ]
};
...
const Codeide = () => {
  ...
  // Terminal State
  const [terminalLines, setTerminalLines] = useState([{ type: 'system', content: 'Strict Compiler Mode Ready...' }]);
  const [isWaitingForInput, setIsWaitingForInput] = useState(false);
  const [userInput, setUserInput] = useState('');
  
  // AI Assistant & Preview State
  const [activeTab, setActiveTab] = useState('terminal'); // 'terminal' | 'assistant' | 'preview'
  const [assistantOutput, setAssistantOutput] = useState('Select an AI tool above to analyze your code.');
  ...
  const styles = {
    ...
    container: isDark ? 'bg-black/40 border-white/10 shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)]' : 'bg-white/80 border-slate-300 shadow-xl',
    sidebar: isDark ? 'bg-black/40 border-white/5' : 'bg-white/50 border-slate-200',
    inputBg: isDark ? 'bg-white/5 border-white/10 focus-within:border-indigo-500/50' : 'bg-white border-slate-300 focus-within:border-indigo-500',
    ...
    highlight: isDark ? 'selection:bg-indigo-500/30' : 'selection:bg-indigo-200',
    modal: isDark ? 'bg-[#0c0c16]/90 border-white/10 text-slate-200' : 'bg-white/95 border-slate-200 text-slate-800'
  };
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Tab') {
      ...
      const insert = "  ";
      setCode(prev => prev.substring(0, start) + insert + prev.substring(end));
...
// --- API HANDLER ---
  const callGemini = async (messages, history, mode = 'compiler') => {
    ...
    const systemPrompt = mode === 'compiler' ? COMPILER_SYSTEM_PROMPT : ASSISTANT_SYSTEM_PROMPT;
    ...
    try {
      ...
      if (mode === 'compiler') {
        setTerminalLines(prev => [...prev, { type: 'error', content: errorMsg }]);
  ...
  const handleCompilerResponse = (rawText, oldHistory, sentMessages) => {
    ...
    if (rawText.includes(inputToken)) {
      ...
      if (preInputText) setTerminalLines(prev => [...prev, { type: 'output', content: preInputText }]);
      
      setChatHistory([...sentMessages, { role: "model", parts: [{ text: rawText }] }]);
      setIsWaitingForInput(true);
      setIsRunning(false);
    } else {
      const isError = /Error:|Exception|Traceback|error:|undefined/.test(rawText);
      if (rawText) {
        setTerminalLines(prev => [...prev, { type: isError ? 'error' : 'output', content: rawText }]);
      }
      setChatHistory([...sentMessages, { role: "model", parts: [{ text: rawText }] }]);
      setIsRunning(false);
      ...
      addToHistory(runningCodeRef.current, rawText, isError ? 'Error' : 'Success');
    }
  };
  ...
  const addToHistory = (codeSnap, outputSnap, status) => {
    setHistory(prev => [{
      ...
      code: codeSnap,
      output: outputSnap.substring(0, 100) + (outputSnap.length > 100 ? '...' : ''),
      status
    }, ...prev]);
  };
  ...
  const handleRun = () => {
    ...
    const initialMessage = [{ role: "user", parts: [{ text: `Execute this code:\n\`\`\`\n${code}\n\`\`\`\n` }] }];
  ...
  const restoreFromHistory = (entry) => {
    ...
    setTerminalLines([{ type: 'system', content: `Restored code from ${entry.timestamp}` }]);
  };
  ...
  const handleAssistantAction = (actionType) => {
    ...
    let prompt = "";
    switch(actionType) {
      case 'explain': prompt = `Explain the following code step-by-step in simple terms:\n\`\`\`\n${code}\n\`\`\``; break;
      case 'fix': prompt = `Analyze the following code for logical and syntax errors. If found, explain them and provide the corrected code:\n\`\`\`\n${code}\n\`\`\``; break;
      case 'docs': prompt = `Add comprehensive comments and docstrings to the following code. Return the full code block with comments:\n\`\`\`\n${code}\n\`\`\``; break;
      case 'complexity': prompt = `Analyze the Time and Space complexity of this code. Explain why:\n\`\`\`\n${code}\n\`\`\``; break;
      case 'review': prompt = `Perform a strict Code Review on this snippet. Focus on code style, best practices, variable naming, and readability. Suggest refactoring if necessary:\n\`\`\`\n${code}\n\`\`\``; break;
      case 'tests': prompt = `Generate a set of Unit Tests for this code. Use the standard testing framework for the language (e.g., pytest for Python, Jest for JS, GTest for C++). Provide the test code in a code block:\n\`\`\`\n${code}\n\`\`\``; break;
      case 'quiz': prompt = `Create a single multiple-choice question (with 4 options) to test my understanding of this specific code snippet. Provide the question, the options, and then the hidden answer/explanation at the very bottom:\n\`\`\`\n${code}\n\`\`\``; break;
      default: prompt = `Analyze this code:\n\`\`\`\n${code}\n\`\`\``;
    }
  ...
  const handleInputSubmit = (e) => {
    ...
    setTerminalLines(prev => [...prev, { type: 'input', content: userInput }]);
    const newMessages = [...chatHistory, { role: "user", parts: [{ text: userInput }] }];
  ...
  const loadExample = (key) => {
    ...
    setTerminalLines([{ type: 'system', content: `Loaded example: ${EXAMPLE_PROGRAMS[key].label}` }]);
    setIsWaitingForInput(false);
    setIsRunning(false);
  };
  ...
  const renderHighlightedCode = useMemo(() => {
    ...
    return tokens.map((token, index) => {
      ...
      if (isDark) {
        switch (token.type) {
          case 'keyword': colorClass = 'text-pink-400 font-bold'; break;
          case 'string': colorClass = 'text-emerald-300'; break;
          case 'comment': colorClass = 'text-slate-500 italic'; break;
          case 'function': colorClass = 'text-blue-300'; break;
          case 'number': colorClass = 'text-orange-300'; break;
          case 'boolean': colorClass = 'text-purple-400'; break;
          case 'tag': colorClass = 'text-blue-400 font-semibold'; break; // New for HTML
        }
      } else {
        // Light Mode Syntax Colors
        switch (token.type) {
          case 'keyword': colorClass = 'text-purple-700 font-bold'; break;
          case 'string': colorClass = 'text-green-700'; break;
          case 'comment': colorClass = 'text-slate-400 italic'; break;
          case 'function': colorClass = 'text-blue-600'; break;
          case 'number': colorClass = 'text-orange-600'; break;
          case 'boolean': colorClass = 'text-red-600'; break;
          case 'tag': colorClass = 'text-blue-700 font-semibold'; break; // New for HTML
        }
      }
      return <span key={index} className={colorClass}>{token.content}</span>;
    });
  }, [code, selectedExample, isDark]);

  return (
    <div className={`flex items-center justify-center h-screen w-full ${styles.bg} overflow-hidden p-2 md:p-6 font-sans transition-colors duration-500`}>
      {/* BACKGROUND EFFECTS */}
      <div className="fixed inset-0 z-0">
          ...
          {isDark && <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:50px_50px] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_50%,#000_70%,transparent_100%)]"></div>}
      </div>

      {/* HISTORY MODAL */}
      {showHistoryModal && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setShowHistoryModal(false)}>
          <div className={`w-full max-w-2xl h-[80vh] flex flex-col rounded-2xl border shadow-2xl overflow-hidden ${styles.modal}`} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-white/10">
              <h2 className="text-lg font-bold flex items-center gap-2"><History size={20} /> Execution History</h2>
              <button onClick={() => setShowHistoryModal(false)} className="p-1 hover:bg-black/10 rounded"><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {history.length === 0 ? (
                <div className="text-center opacity-50 py-10">No execution history yet.</div>
              ) : (
                history.map(entry => (
                  <div key={entry.id} className={`p-3 rounded-lg border flex flex-col gap-2 ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="flex items-center justify-between text-xs opacity-70">
                      <span className="flex items-center gap-1"><Clock size={12} /> {entry.timestamp}</span>
                      <span className={entry.status === 'Error' ? 'text-red-400' : 'text-green-400'}>{entry.status}</span>
                    </div>
                    <div className="font-mono text-xs opacity-80 truncate">{entry.output}</div>
                    <button 
      ...
      <div className={`relative z-10 flex w-full max-w-[1600px] h-full max-h-[1000px] backdrop-blur-2xl rounded-[30px] border overflow-hidden ring-1 ring-white/5 transition-colors duration-500 ${styles.container}`}>
        
        {/* SIDEBAR NAVIGATION */}
        <div className={`w-20 border-r flex flex-col items-center py-8 gap-6 ${styles.sidebar}`}>
            <div 
                onClick={() => navigate('/')} 
            ...
            >
                <Cpu size={24} className="text-white group-hover:rotate-90 transition-transform duration-500" />
            </div>
            ...
            <div className="flex-1 flex flex-col gap-6 w-full items-center">
                ...
                >
                    {isRunning ? <Loader2 size={20} className="text-white/50 animate-spin" /> : <Play size={20} className="text-white fill-current ml-1" />}
                ...
                <div className={`h-px w-8 ${isDark ? 'bg-white/10' : 'bg-slate-300'}`}></div>
                ...
                {/* Example Loader Trigger */}
                 <div className="relative group">
                    <button className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all hover:scale-110 ${isDark ? 'bg-white/5 text-slate-400 hover:text-white' : 'bg-slate-200 text-slate-600 hover:text-slate-900'}`}>
                        <Code2 size={20} />
                    </button>
                    {/* Floating Menu */}
                    <div className={`absolute top-0 left-12 w-64 border rounded-xl p-2 opacity-0 group-hover:opacity-100 transition-all pointer-events-none group-hover:pointer-events-auto transform origin-left scale-95 group-hover:scale-100 shadow-2xl z-50 ${isDark ? 'bg-[#0a0a16] border-white/10' : 'bg-white border-slate-200'}`}>
                        <div className="text-xs font-bold text-slate-500 uppercase px-3 py-2">Load Example</div>
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-transparent relative group/editor">
                ...
                {/* STUDENT INFO BAR */}
                <div className={`flex items-center gap-4 p-3 border-b backdrop-blur-md z-40 transition-colors ${isDark ? 'bg-black/40 border-white/10' : 'bg-white/50 border-slate-200'}`}>
                    <div className={`flex items-center gap-2 rounded-lg px-3 py-1.5 border transition-colors ${styles.inputBg}`}>
                ...
                {/* FLOATING TOOLBAR */}
                <div className={`absolute top-16 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1 p-1.5 backdrop-blur-xl border rounded-full shadow-2xl scale-90 md:scale-100 opacity-50 hover:opacity-100 transition-all ${isDark ? 'bg-black/60 border-white/10' : 'bg-white/80 border-slate-200'}`}>
                    {[
                        { id: 'explain', icon: Sparkles, color: 'text-amber-400', label: 'Explain' },
                        { id: 'fix', icon: Bug, color: 'text-rose-400', label: 'Debug' },
                        { id: 'docs', icon: FileText, color: 'text-blue-400', label: 'Docs' },
                        { id: 'complexity', icon: Activity, color: 'text-emerald-400', label: 'Analysis' },
                        { id: 'review', icon: ShieldCheck, color: 'text-purple-400', label: 'Review' },
                    ].map(tool => (
                        <button 
                        key={tool.id}
                        ...
                        >
                            ...
                            <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[10px] bg-black/80 text-white px-2 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{tool.label}</span>
                        </button>
                    ))}
                    <div className={`w-px h-4 mx-1 ${isDark ? 'bg-white/20' : 'bg-slate-300'}`}></div>
                    <button onClick={() => handleAssistantAction('quiz')} className="px-3 py-1.5 rounded-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white text-xs font-bold flex items-center gap-1.5 shadow-lg shadow-indigo-500/25 transition-transform active:scale-95">
                ...
                <div className="flex-1 flex relative mt-0 bg-transparent overflow-hidden">
                 ...
                 <div className={`h-8 border-t flex items-center px-6 text-xs font-mono transition-colors ${isDark ? 'bg-black/20 border-white/5 text-slate-500' : 'bg-slate-100 border-slate-200 text-slate-600'}`}>
                    <span className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${isRunning ? 'bg-amber-400 animate-pulse' : 'bg-emerald-500'}`}></div> {isRunning ? 'Compiling...' : 'Ready'}</span>
                    <span className="mx-4 opacity-20">|</span>
                    <span>{EXAMPLE_PROGRAMS[selectedExample].lang.toUpperCase()}</span>
                    
                    {/* Font Size Controls */}
                    <div className="flex items-center gap-2 ml-4">
                        <button onClick={() => setFontSize(p => Math.max(10, p - 1))} className={`p-0.5 rounded hover:bg-current/10 transition-colors`}><ZoomOut size={12} /></button>
                        <span>{fontSize}px</span>
                        <button onClick={() => setFontSize(p => Math.min(24, p + 1))} className={`p-0.5 rounded hover:bg-current/10 transition-colors`}><ZoomIn size={12} /></button>
                    </div>

                    <div className="flex-1"></div>
                    <span>Ln {code.substring(0, textAreaRef.current?.selectionStart).split("\n").length}, Col {textAreaRef.current?.selectionStart - code.lastIndexOf("\n", textAreaRef.current?.selectionStart - 1)}</span>
                </div>
            </div>

            {/* RIGHT PANEL (TERMINAL / ASSISTANT / PREVIEW) */}
            <div className={`w-full md:w-[450px] border-l flex flex-col backdrop-blur-md transition-colors ${isDark ? 'bg-[#0c0c16]/50 border-white/5' : 'bg-white/40 border-slate-200'}`}>
                ...
                <div className="flex p-4 gap-3">
                     {!isWebMode && (
                         <button 
                            ...
                            className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 ${
                                activeTab === 'terminal' 
                                ? isDark ? 'bg-slate-800 text-emerald-400 shadow-lg shadow-black/20 ring-1 ring-white/5' : 'bg-white text-emerald-600 shadow-md ring-1 ring-slate-200'
                    ...
                    {isWebMode && (
                         <button 
                            ...
                            className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 ${
                                activeTab === 'preview' 
                                ? isDark ? 'bg-blue-900/40 text-blue-400 shadow-lg shadow-black/20 ring-1 ring-white/5' : 'bg-white text-blue-600 shadow-md ring-1 ring-slate-200'
                                : 'bg-transparent text-slate-500 hover:bg-current/5'
                            }`}
                        >
                    ...
                    <button 
                        ...
                        className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 ${
                            activeTab === 'assistant' 
                            ? isDark ? 'bg-gradient-to-r from-violet-600/20 to-indigo-600/20 text-indigo-300 shadow-lg ring-1 ring-indigo-500/30' : 'bg-white text-indigo-600 shadow-md ring-1 ring-indigo-200'
                            : 'bg-transparent text-slate-500 hover:bg-current/5'
                        }`}
                    >
                ...
                {/* Content Container */}
                <div className={`flex-1 overflow-hidden relative mx-4 mb-4 rounded-2xl border shadow-inner ${isDark ? 'bg-[#050510] border-white/10' : 'bg-[#1e1e2e] border-slate-300'}`}>
                    
                    {/* TERMINAL VIEW */}
                    {activeTab === 'terminal' && !isWebMode && (
                        <div className="absolute inset-0 flex flex-col">
                            <div className="flex justify-end p-2 absolute top-0 right-0 z-10">
                                <button onClick={handleClearTerminal} className="p-1.5 text-slate-500 hover:text-rose-400 transition-colors bg-white/5 hover:bg-white/10 rounded-lg">
                                    <Trash2 size={12} />
                                </button>
                            </div>
                            ...
                            >
                                {terminalLines.map((line, idx) => (
                                    <div key={idx} className={`${
                                        line.type === 'error' ? 'text-rose-400 bg-rose-900/10 border-l-2 border-rose-500 pl-2' : 
                                        line.type === 'system' ? 'text-slate-500 italic' : 
                                        line.type === 'input' ? 'text-cyan-300' : 'text-slate-300'
                                    } break-words`}> 
                                ...
                                {isWaitingForInput && (
                                    <form onSubmit={handleInputSubmit} className="flex items-center mt-2 group">
                         ...
                         <div className={`absolute inset-0 flex flex-col p-6 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-900/50 ${isDark ? 'bg-[#050510]' : 'bg-[#f8fafc]'}`}>
                            {isAssistantLoading ? (
                                ...
                                <div className={`prose prose-sm max-w-none ${isDark ? 'prose-invert text-slate-300' : 'text-slate-700'}`}>
                                    <div className="whitespace-pre-wrap">{assistantOutput}</div>
                                </div>
                            ) : (
