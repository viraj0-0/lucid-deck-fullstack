...
branch_pattern = re.compile(r"Choice Code\s*:\s*(\d+)\s*Course Name\s*:\s*(.*)")
category_pattern = re.compile(r"^[A-Z\- ]+$")
percent_pattern = re.compile(r"\((\d+\.\d+)%\)")

def parse_file(path, round_key, colleges):
            ...
            if current_branch and current_categories:
                ...
                if percents:
                    for cat, pct in zip(current_categories, percents):
                        current_branch["cutoffs"][round_key][cat] = float(pct)
                    current_categories = []

colleges = {}

parse_file(BASE_DIR / "raw_cap1.txt", "CAP1", colleges)
parse_file(BASE_DIR / "raw_cap2.txt", "CAP2", colleges)
...
print("✅ college_data.json generated with CAP1 & CAP2 separation")
